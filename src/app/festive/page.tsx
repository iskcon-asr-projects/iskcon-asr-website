...
export default function FestiveDarshanPage() {
  ...
  const [selectedFestivalId, setSelectedFestivalId] = useState<string | null>(rawIdParam);
  ...
  // Fetch recent festivals for the top navigation bar
  useEffect(() => {
    const fetchRecent = async () => {
      try {
        const res = await fetch("/api/festive-darshans/recent");
        const json = await res.json();
        if (json.data && json.data.length > 0) {
          setRecentFestivals(json.data);
          
          if (!rawIdParam) {
            setSelectedFestivalId(json.data[0].id);
            setSelectedFestival(json.data[0]);
          } else {
            const matched = json.data.find((f: Festival) => f.id === rawIdParam);
            if (matched) setSelectedFestival(matched);
          }
        } else {
          setIsLoading(false); // Entire DB might be empty
        }
      } catch (err) {
        console.error(err);
      }
  ...
  // Fetch specific festival images when selection changes
  useEffect(() => {
    ...
    const fetchDarshan = async () => {
      setIsLoading(true);
      try {
        // Since we didn't add a GET by ID in festive-darshans API route (only date or festival_name), 
        // we can fetch by festival_name to get the current items. Alternatively, we modify the GET.
        // The current GET supports ?date= or ?festival_name=. But we can just grab all and filter, 
        // or update our GET logic. For simplicity, we just use the festival_name if id is not directly supported 
        // by the GET query yet, or we fetch all recent and find. Let's fetch all (default limit 20) and find by ID.
        // We know /api/festive-darshans will return the records.
        const res = await fetch(`/api/festive-darshans`);
        const json = await res.json();
        
        if (json.data && json.data.length > 0) {
          const specificFestival = json.data.find((row: Festival) => row.id === selectedFestivalId);
          if (specificFestival) {
            setImages(specificFestival.images || []);
            setSelectedFestival(specificFestival);
          } else {
            setImages([]);
          }
        } else {
  ...
  const handleFestivalClick = (festival: Festival) => {
    ...
    // Optionally update URL to allow sharing specific festivals
    router.replace(`/festive?id=${festival.id}`, { scroll: false });
  };
      ...
      <main className="relative z-10 pt-32 pb-32 px-4 md:px-12 lg:px-24 max-w-[2000px] mx-auto min-h-screen flex flex-col items-center">
        ...
        <header className="w-full max-w-6xl flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <h1 className="font-['Noto_Serif'] font-light text-5xl md:text-7xl text-white mb-4 tracking-tight drop-shadow-lg">
              Festive <span className="italic text-[#C5A059]">Darshans</span>
            </h1>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1 }} className="text-left md:text-right">
            <p className="font-['Manrope'] uppercase font-semibold tracking-[0.3em] text-[#C5A059]/60 text-[10px] mb-2">Sacred Gathering</p>
            <AnimatePresence mode="wait">
              <motion.div key={selectedFestivalId || "empty"} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                <p className="font-['Noto_Serif'] text-xl md:text-2xl text-white/90">
                  {selectedFestival ? selectedFestival.festival_name : "Awaiting Selection"}
                </p>
                {selectedFestival && (
                  <p className="text-white/40 text-sm mt-1">{formatFullDate(selectedFestival.date)}</p>
                )}
              </motion.div>
            </AnimatePresence>
          </motion.div>
        ...
        <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="w-full max-w-6xl mb-16 relative">
          <div className="relative border-y border-white/10 py-5 flex items-center gap-8 overflow-x-auto no-scrollbar mask-edges">
            <span className="font-['Manrope'] text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] text-white/40 whitespace-nowrap pl-2">Past Festivals</span>
            {recentFestivals.length === 0 && <span className="text-white/20 text-xs italic">No festivals available</span>}
            
            <div className="flex gap-6 pr-6">
              {recentFestivals.map((f, index) => {
                const isActive = f.id === selectedFestivalId;
                return (
                  <button
                    key={f.id}
                  ...
                  >
                    {index === 0 && !rawIdParam ? `Latest (${f.festival_name})` : f.festival_name}
                  </button>
                );
              })}
            </div>
        ...
        {isLoading ? (
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-2xl py-32 text-center bg-white/5 border border-white/5 rounded-none backdrop-blur-sm shadow-2xl">
            ...
            <p className="font-['Manrope'] text-white/40 text-sm mb-8">The sacred viewing for this festival has not been released yet.</p>
            <button onClick={() => router.push('/admin/festive-darshan')} className="font-['Manrope'] text-xs uppercase font-bold tracking-[0.2em] text-[#C5A059] bg-[#C5A059]/10 hover:bg-[#C5A059]/20 px-8 py-4 rounded-none transition-all">Go to Upload Portal</button>
          </motion.div>
        ) : (
          <div className="w-full">
            ...
            {/* Flexible Centered Layout for Infinite Scaling Images */}
            <motion.div 
              variants={containerVariants} 
              initial="hidden" 
              animate="show"
              key={selectedFestivalId} 
                ...
                .map((img, i) => (
                  ...
                  >
                    <div className="w-full relative overflow-hidden bg-black flex items-center justify-center min-h-[300px]">
                      <Image 
                        src={img.url!} 
                        alt={`${selectedFestival?.festival_name || 'Festive'} Darshan - Upload ${i + 1}`}
                        fill
