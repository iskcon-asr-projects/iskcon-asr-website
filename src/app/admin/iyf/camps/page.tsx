...
export default function AdminCamps() {
  ...
  // Derive unique camp names from existing posts
  const existingCampNames = useMemo(() => {
    const names = new Map<string, { date: string; count: number }>();
    posts.forEach((p) => {
      if (names.has(p.camp_name)) {
        names.get(p.camp_name)!.count++;
      } else {
        names.set(p.camp_name, { date: p.date || "", count: 1 });
      }
    });
    return names;
  }, [posts]);

  // Group posts by camp name
  const groupedPosts = useMemo(() => {
    const groups = new Map<string, CampPost[]>();
    posts.forEach((p) => {
      if (!groups.has(p.camp_name)) {
        groups.set(p.camp_name, []);
      }
      groups.get(p.camp_name)!.push(p);
    });
    return groups;
  }, [posts]);
  ...
  const fetchPosts = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/iyf/camps");
  ...
  const addPost = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.camp_name || !form.content) {
      alert("Please enter camp name and content (IG link/code).");
      return;
    }
    ...
    <div className="min-h-screen bg-[#050505] text-white p-6 md:p-12 font-sans">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <header className="mb-12 border-b border-white/10 pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-4xl text-[#C5A059] font-serif mb-2 tracking-tight">Manage IYF Camps</h1>
            <p className="text-white/40 text-sm tracking-wide">Document our spiritual camps and retreats.</p>
          </div>
          <div className="flex gap-4 flex-wrap">
            ...
            <button onClick={() => router.push('/admin/iyf/camps')} className="text-[10px] uppercase tracking-[0.2em] border border-[#C5A059]/40 rounded-none text-[#C5A059] px-4 py-2 transition-colors">Camps</button>
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="bg-[#0A0A0A] border border-white/5 rounded-none p-8 shadow-2xl">
              ...
              <form onSubmit={addPost} className="space-y-6">
                <div>
                  <label className="block text-[10px] uppercase tracking-[0.2em] text-white/40 mb-3 font-bold">Select Camp</label>
                  <div className="grid grid-cols-1 gap-3">
                    {Array.from(existingCampNames.keys()).map((name) => (
                      <button
                        key={name}
                        type="button"
                        onClick={() => {
                          setForm({ ...form, camp_name: name, date: existingCampNames.get(name)?.date || "" });
                          setIsNewCamp(false);
                        }}
                        className={`text-left px-4 py-3 rounded-none border transition-all ${
                          form.camp_name === name && !isNewCamp
                            ? "bg-[#C5A059]/10 border-[#C5A059]/40 text-[#C5A059]"
                            : "bg-white/5 border-white/5 text-white/60 hover:border-white/20"
                        }`}
                      >
                        <div className="text-sm font-medium">{name}</div>
                        <div className="text-[10px] opacity-50 uppercase tracking-wider">{existingCampNames.get(name)?.date || "No date"}</div>
                      </button>
                    ))}
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span className="text-[10px] uppercase tracking-widest font-bold">Add New Camp</span>
                    </button>
                  </div>
                </div>
                ...
                {isNewCamp && (
                  <div className="space-y-6 overflow-hidden">
                    <div className="space-y-4 pt-4 border-t border-white/5">
                      <div>
                        <label className="block text-[10px] uppercase tracking-[0.2em] text-white/40 mb-3 font-bold">New Camp Name</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-[10px] uppercase tracking-[0.2em] text-white/40 mb-3 font-bold">Camp Date (Optional)</label>
                        <input
                          type="text"
                          placeholder="e.g. Jan 15-18, 2026"
                          value={form.date}
                ...
                <div>
                  <label className="block text-[10px] uppercase tracking-[0.2em] text-white/40 mb-3 font-bold">Type</label>
                  <div className="flex gap-4">
                      <button
                        type="button"
                        onClick={() => setForm({ ...form, type: "instagram_link" })}
                        className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-none border transition-all ${
                          form.type === "instagram_link"
                            ? "bg-[#C5A059]/10 border-[#C5A059]/40 text-[#C5A059]"
                            : "bg-white/5 border-white/5 text-white/40 hover:border-white/20"
                        }`}
                      >
                        <Camera size={20} />
                        <span className="text-[10px] uppercase tracking-widest font-bold">IG Link</span>
                      </button>
                    <button
                      type="button"
                      onClick={() => setForm({ ...form, type: "instagram" })}
                      className={`flex-1 flex flex-col items-center gap-2 p-4 rounded-none border transition-all ${
                        form.type === "instagram"
                          ? "bg-[#C5A059]/10 border-[#C5A059]/40 text-[#C5A059]"
                          : "bg-white/5 border-white/5 text-white/40 hover:border-white/20"
                    ...
                    >
                      <LayoutGrid size={20} />
                      <span className="text-[10px] uppercase tracking-widest font-bold">Embed Code</span>
                    </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    rows={4}
                    placeholder={form.type === "instagram_link" ? "Paste Instagram URL here..." : "Paste <blockquote> embed code here..."}
                    value={form.content}
                ...
                <button
                  ...
                  className="w-full py-5 rounded-none bg-[#C5A059] text-black font-bold uppercase tracking-[0.2em] text-xs hover:bg-[#D5B069] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-xl shadow-[#C5A059]/10"
                >
                  {isUploading ? "Publishing..." : "Add Post — Camps"}
                </button>
              </form>
            </div>
          </div>

          {/* RIGHT: LIST OF POSTS GROUPED BY CAMP */}
          <div className="lg:col-span-7 space-y-10">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                <div className="w-8 h-8 border-2 border-[#C5A059]/20 border-t-[#C5A059] rounded-none animate-spin" />
                <span className="text-[10px] text-white/40 uppercase tracking-widest font-bold">Loading Posts...</span>
              </div>
            ) : posts.length === 0 ? (
              <div className="text-white/20 text-center py-20 text-sm tracking-widest border border-dashed border-white/5 rounded-none">NO CAMP POSTS YET</div>
            ) : (
              Array.from(groupedPosts.entries()).map(([camp, items]) => (
                <div key={camp} className="bg-[#0A0A0A] border border-white/5 rounded-none overflow-hidden shadow-2xl">
                  <div className="px-6 py-4 border-b border-white/5 bg-white/[0.01] flex items-center justify-between">
                    <div>
                      <h3 className="text-lg font-serif text-white">{camp}</h3>
                      <p className="text-[10px] text-white/30 uppercase tracking-wider">{items[0]?.date || "No date"}</p>
                    </div>
                      ...
                      <div key={post.id} className="p-6 flex items-center justify-between group hover:bg-white/[0.02] transition-colors">
                        <div className="flex items-center gap-4 min-w-0 flex-1 pr-4">
                          <div className="w-10 h-10 rounded-none bg-white/5 flex items-center justify-center shrink-0">
                            {post.type === "instagram" || post.type === "instagram_link" ? <Camera size={16} className="text-pink-500/50" /> : <Type size={16} className="text-blue-500/50" />}
                          </div>
                          <div className="min-w-0">
                            <div className="text-xs text-white/70 truncate font-mono bg-black/30 px-2 py-1 rounded-none">
                              {post.content.substring(0, 40)}...
                            </div>
                            <div className="text-[10px] text-white/20 mt-1 uppercase tracking-widest font-bold">
