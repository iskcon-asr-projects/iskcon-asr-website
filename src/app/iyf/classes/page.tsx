...
export default function ClassesGalleryPage() {
  ...
  // Group by Course/Series Title
  const groupedSeries = useMemo(() => {
    const map = new Map<string, ClassPost[]>();
    posts.forEach(p => {
      if (!map.has(p.class_title)) map.set(p.class_title, []);
      map.get(p.class_title)!.push(p);
    });
      ...
      <div className="pt-32 pb-20 px-6 md:px-12 max-w-[1400px] mx-auto">
        ...
        <header className="mb-20">
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="text-orange-500 text-[10px] uppercase tracking-[0.4em] font-bold">Spiritual Education</span>
          </div>
          <h1 className="text-5xl md:text-7xl font-serif text-white mb-8">
            Classes <span className="text-orange-400 italic">Archive</span>
          </h1>
          <p className="text-white/40 max-w-2xl text-lg font-light leading-relaxed">
            Relive the wisdom shared in our IYF Classes, Gita workshops, and interactive seminars.
          </p>
        </header>
        ...
        {isLoading ? (
          ...
          <div className="py-40 text-center border border-dashed border-white/10 rounded-none">
            <BookOpen size={48} className="text-white/10 mx-auto mb-6" />
            <p className="text-white/30 text-sm tracking-widest uppercase">No classes archived yet.</p>
          </div>
        ) : (
          <div className="space-y-32">
            {groupedSeries.map(([title, items]) => (
              <section key={title} className="relative">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                  <div>
                    <h2 className="text-3xl md:text-5xl font-serif text-white/90 mb-2">{title}</h2>
                    <div className="flex items-center gap-2 text-orange-400/60 font-medium tracking-wide">
                      <Calendar size={14} />
                      <span className="text-sm">{items[0]?.date || "Spiritual Series"}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 text-white/30 bg-white/5 px-4 py-2 rounded-none border border-white/10">
                    <span className="text-[10px] font-bold uppercase tracking-wider">{items.length} Lessons</span>
                  </div>
                </div>
